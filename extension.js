...
function FindText() {
    ...
    let nick = null;
    //let parent = null;
    let abort = false; // if set to true, the program should abort any current attempt to inject time data into a message
    let words = null, word = "";
    let newMessageText = "";
...
function IsSupportedMessage() {
    if (messagetext.includes("-g")) {
        ...
        return false; // for now at least, I might add support later for -g
    } else {
        return true;
    }
}
...
function CalcTotSeconds(lightSeconds) {
    ...
    if (lightSeconds < 100000) {    
      return(Math.floor(Math.pow(lightSeconds, 0.3292)  * 8.9034));
    }
    else
    if (lightSeconds < 1907087) {
      ...
      return(Math.floor((lightSeconds - 5265389.609) / 2001 + 3412));
    }
}
